import type { IDatabaseDriver, VectorEngine, ChunkRow, SearchResult, BaseMetadata } from "../types";

/**
 * PGLite (pgvector) 向けのベクトル操作エンジン
 */
export class PGLiteVectorEngine<T extends BaseMetadata = BaseMetadata> implements VectorEngine<T> {
  private driver: IDatabaseDriver;
  private embeddingDim: number;

  constructor(driver: IDatabaseDriver, embeddingDim: number) {
    this.driver = driver;
    this.embeddingDim = embeddingDim;
  }

  async initSchema(): Promise<void> {
    await this.driver.exec("CREATE EXTENSION IF NOT EXISTS vector;");
  ...
  async insertChunk(chunk: ChunkRow<T>): Promise<void> {
    ...
    const stmt = await this.driver.prepare(
  ...
  async bulkInsertChunks(chunks: ChunkRow<T>[], batchSize: number): Promise<void> {
    const insertMany = this.driver.transaction(async (batch: ChunkRow<T>[]) => {
      const stmt = await this.driver.prepare(
        `INSERT INTO chunks (content, filepath, metadata, embedding) VALUES ($1, $2, $3, $4)`
      );
